...
[[cpp11::register]]
SEXP dust_cpu_seirhdagevaxmultistrainserotimedepbeta_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
...
[[cpp11::register]]
SEXP dust_cpu_seirhdagevaxmultistrainserotimedepbeta_update_state(SEXP ptr, SEXP r_pars, SEXP r_state,
...
[[cpp11::register]]
SEXP dust_cpu_seirhdagevaxmultistrainserotimedepbeta_filter(SEXP ptr, SEXP step_end,
...
// [[dust::class(seirhdagevaxmultistrainserotimedepbeta)]]
...
class seirhdagevaxmultistrainserotimedepbeta {
public:
  ...
  struct shared_type {
    ...
    real_type initial_cases_non_variant_inc;
  ...
  seirhdagevaxmultistrainserotimedepbeta(const dust::pars_type<seirhdagevaxmultistrainserotimedepbeta>& pars) :
    shared(pars.shared), internal(pars.internal) {
  }
  size_t size() {
    return shared->dim_D + shared->dim_E + shared->dim_G + shared->dim_H + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_T_N_1 + shared->dim_T_P_1 + shared->dim_T_pre_1 + shared->dim_vaccine_missed_doses + 22;
  }
  std::vector<real_type> initial(size_t step) {
    std::vector<real_type> state(shared->dim_D + shared->dim_E + shared->dim_G + shared->dim_H + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_T_N_1 + shared->dim_T_P_1 + shared->dim_T_pre_1 + shared->dim_vaccine_missed_doses + 22);
    ...
    state[20] = shared->initial_cases_inc;
    state[21] = shared->initial_cases_non_variant_inc;
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    const real_type cases_inc = state[20];
    const real_type cases_non_variant_inc = state[21];
    ...
    real_type new_cases = odin_sum1<real_type>(internal.n_EI_P.data(), 0, shared->dim_n_EI_P);
    real_type new_cases_non_variant = odin_sum3<real_type>(internal.n_EI_P.data(), 0, shared->dim_n_EI_P_1, 0, 1, 0, shared->dim_n_EI_P_3, shared->dim_n_EI_P_1, shared->dim_n_EI_P_12) + ((shared->n_real_strains == 2 ? odin_sum3<real_type>(internal.n_EI_P.data(), 0, shared->dim_n_EI_P_1, 3, 4, 0, shared->dim_n_EI_P_3, shared->dim_n_EI_P_1, shared->dim_n_EI_P_12) : 0));
    ...
    state_next[20] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_cases : cases_inc + new_cases));
    state_next[21] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_cases_non_variant : cases_non_variant_inc + new_cases_non_variant));
    ...
    for (int i = 1; i <= shared->dim_p_SE_1; ++i) {
      for (int j = 1; j <= shared->dim_p_SE_2; ++j) {
        internal.p_SE[i - 1 + shared->dim_p_SE_1 * (j - 1)] = 1 - std::exp(- odin_sum3<real_type>(internal.lambda_sus.data(), i - 1, i, 0, shared->dim_lambda_sus_2, j - 1, j, shared->dim_lambda_sus_1, shared->dim_lambda_sus_12) * shared->dt);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_rel_foi_strain_1; ++i) {
      for (int j = 1; j <= shared->dim_rel_foi_strain_2; ++j) {
        for (int k = 1; k <= shared->dim_rel_foi_strain_3; ++k) {
          internal.rel_foi_strain[i - 1 + shared->dim_rel_foi_strain_1 * (j - 1) + shared->dim_rel_foi_strain_12 * (k - 1)] = ((odin_sum3<real_type>(internal.lambda_sus.data(), i - 1, i, 0, shared->dim_lambda_sus_2, k - 1, k, shared->dim_lambda_sus_1, shared->dim_lambda_sus_12) == 0 ? ((j == 1 ? 1 : 0)) : std::min(internal.lambda_sus[shared->dim_lambda_sus_12 * (k - 1) + shared->dim_lambda_sus_1 * (j - 1) + i - 1] / (real_type) odin_sum3<real_type>(internal.lambda_sus.data(), i - 1, i, 0, shared->dim_lambda_sus_2, k - 1, k, shared->dim_lambda_sus_1, shared->dim_lambda_sus_12), static_cast<real_type>(1))));
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_p_Rx_1; ++i) {
      for (int j = 1; j <= shared->dim_p_Rx_2; ++j) {
        for (int k = 1; k <= shared->dim_p_Rx_3; ++k) {
          internal.p_Rx[i - 1 + shared->dim_p_Rx_1 * (j - 1) + shared->dim_p_Rx_12 * (k - 1)] = 1 - std::exp(- internal.rate_Rx[shared->dim_rate_Rx_12 * (k - 1) + shared->dim_rate_Rx_1 * (j - 1) + i - 1] * shared->dt);
        }
      }
    }
    ...
    }
    {
       ...
       int j = 1;
       int k = 1;
       internal.n_SE[i - 1 + shared->dim_n_SE_1 * (j - 1) + shared->dim_n_SE_12 * (k - 1)] = internal.n_SE[shared->dim_n_SE_12 * 0 + shared->dim_n_SE_1 * 0 + shared->seed_age - 1] + std::min(S[shared->dim_S_1 * 0 + shared->seed_age - 1] - internal.n_SE_tot[shared->dim_n_SE_tot_1 * 0 + shared->seed_age - 1], seed);
    }
    {
       int i = shared->seed_age;
       for (int j = 2; j <= shared->n_strains; ++j) {
         int k = 1;
         internal.n_SE[i - 1 + shared->dim_n_SE_1 * (j - 1) + shared->dim_n_SE_12 * (k - 1)] = (j < 3 ? std::min(internal.n_SE[shared->dim_n_SE_12 * (k - 1) + shared->dim_n_SE_1 * (j - 1) + i - 1] + S[shared->dim_S_1 * (k - 1) + i - 1] - odin_sum3<real_type>(internal.n_SE.data(), i - 1, i, 0, shared->dim_n_SE_2, k - 1, k, shared->dim_n_SE_1, shared->dim_n_SE_12), internal.n_SE[shared->dim_n_SE_12 * (k - 1) + shared->dim_n_SE_1 * (j - 1) + i - 1] + strain_seed) : 0);
       }
    }
...
namespace dust {
template<>
dust::pars_type<seirhdagevaxmultistrainserotimedepbeta> dust_pars<seirhdagevaxmultistrainserotimedepbeta>(cpp11::list user) {
  using real_type = typename seirhdagevaxmultistrainserotimedepbeta::real_type;
  auto shared = std::make_shared<seirhdagevaxmultistrainserotimedepbeta::shared_type>();
  seirhdagevaxmultistrainserotimedepbeta::internal_type internal;
  ...
  shared->initial_cases_non_variant_inc = 0;
  ...
  shared->p_P_1 = 0.84999999999999998;
  shared->seed_age = 4;
  shared->theta_A = 0.5;
  ...
  shared->p_T_P_1T_N_1 = 1 - std::exp(- shared->gamma_P_1 * shared->dt);
  shared->p_T_pre_1x = 1 - std::exp(- shared->gamma_pre_1 * shared->dt);
  ...
  shared->offset_variable_S = shared->dim_N_tot + 22;
  ...
  shared->offset_variable_D = shared->dim_E + shared->dim_G + shared->dim_H + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_vaccine_missed_doses + 22;
  shared->offset_variable_E = shared->dim_N_tot + shared->dim_S + shared->dim_vaccine_missed_doses + 22;
  shared->offset_variable_G = shared->dim_E + shared->dim_H + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_vaccine_missed_doses + 22;
  shared->offset_variable_H = shared->dim_E + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_vaccine_missed_doses + 22;
  shared->offset_variable_I_A = shared->dim_E + shared->dim_I_P + shared->dim_N_tot + shared->dim_S + shared->dim_vaccine_missed_doses + 22;
  shared->offset_variable_I_C = shared->dim_E + shared->dim_I_A + shared->dim_I_P + shared->dim_N_tot + shared->dim_S + shared->dim_vaccine_missed_doses + 22;
  shared->offset_variable_I_P = shared->dim_E + shared->dim_N_tot + shared->dim_S + shared->dim_vaccine_missed_doses + 22;
  shared->offset_variable_R = shared->dim_E + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_S + shared->dim_vaccine_missed_doses + 22;
  shared->offset_variable_T_N_1 = shared->dim_D + shared->dim_E + shared->dim_G + shared->dim_H + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_T_P_1 + shared->dim_T_pre_1 + shared->dim_vaccine_missed_doses + 22;
  shared->offset_variable_T_P_1 = shared->dim_D + shared->dim_E + shared->dim_G + shared->dim_H + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_T_pre_1 + shared->dim_vaccine_missed_doses + 22;
  shared->offset_variable_T_pre_1 = shared->dim_D + shared->dim_E + shared->dim_G + shared->dim_H + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_vaccine_missed_doses + 22;
  shared->offset_variable_vaccine_missed_doses = shared->dim_N_tot + shared->dim_S + 22;
  ...
  return dust::pars_type<seirhdagevaxmultistrainserotimedepbeta>(shared, internal);
}
template <>
cpp11::sexp dust_info<seirhdagevaxmultistrainserotimedepbeta>(const dust::pars_type<seirhdagevaxmultistrainserotimedepbeta>& pars) {
  const seirhdagevaxmultistrainserotimedepbeta::internal_type internal = pars.internal;
  const std::shared_ptr<const seirhdagevaxmultistrainserotimedepbeta::shared_type> shared = pars.shared;
  cpp11::writable::strings nms({"time", "H_inc", "D_inc", "H_inc_0_39", "H_inc_40_49", "H_inc_50_59", "H_inc_60_69", "H_inc_70_plus", "D_inc_0_39", "D_inc_40_49", "D_inc_50_59", "D_inc_60_69", "D_inc_70_plus", "sero_pos_1", "sero_pos_1_20_29", "sero_pos_1_30_39", "sero_pos_1_40_49", "sero_pos_1_50_59", "sero_pos_1_60_69", "sero_pos_1_70_plus", "cases_inc", "cases_non_variant_inc", "N_tot", "S", "vaccine_missed_doses", "E", "I_P", "I_A", "I_C", "R", "H", "G", "D", "T_pre_1", "T_P_1", "T_N_1"});
  ...
  dim[24] = cpp11::writable::integers({shared->dim_vaccine_missed_doses_1, shared->dim_vaccine_missed_doses_2});
  dim[25] = cpp11::writable::integers({shared->dim_E_1, shared->dim_E_2, shared->dim_E_3});
  dim[26] = cpp11::writable::integers({shared->dim_I_P_1, shared->dim_I_P_2, shared->dim_I_P_3});
  dim[27] = cpp11::writable::integers({shared->dim_I_A_1, shared->dim_I_A_2, shared->dim_I_A_3});
  dim[28] = cpp11::writable::integers({shared->dim_I_C_1, shared->dim_I_C_2, shared->dim_I_C_3});
  dim[29] = cpp11::writable::integers({shared->dim_R_1, shared->dim_R_2, shared->dim_R_3});
  dim[30] = cpp11::writable::integers({shared->dim_H_1, shared->dim_H_2, shared->dim_H_3});
  dim[31] = cpp11::writable::integers({shared->dim_G_1, shared->dim_G_2, shared->dim_G_3});
  dim[32] = cpp11::writable::integers({shared->dim_D_1, shared->dim_D_2, shared->dim_D_3});
  dim[33] = cpp11::writable::integers({shared->dim_T_pre_1_1, shared->dim_T_pre_1_2, shared->dim_T_pre_1_3});
  dim[34] = cpp11::writable::integers({shared->dim_T_P_1_1, shared->dim_T_P_1_2, shared->dim_T_P_1_3});
  dim[35] = cpp11::writable::integers({shared->dim_T_N_1_1, shared->dim_T_N_1_2, shared->dim_T_N_1_3});
  dim.names() = nms;
  ...
  index[24] = integer_sequence(shared->offset_variable_vaccine_missed_doses + 1, shared->dim_vaccine_missed_doses);
  index[25] = integer_sequence(shared->offset_variable_E + 1, shared->dim_E);
  ...
  index[28] = integer_sequence(shared->offset_variable_I_C + 1, shared->dim_I_C);
  index[29] = integer_sequence(shared->offset_variable_R + 1, shared->dim_R);
  index[30] = integer_sequence(shared->offset_variable_H + 1, shared->dim_H);
  index[31] = integer_sequence(shared->offset_variable_G + 1, shared->dim_G);
  index[32] = integer_sequence(shared->offset_variable_D + 1, shared->dim_D);
  index[33] = integer_sequence(shared->offset_variable_T_pre_1 + 1, shared->dim_T_pre_1);
  index[34] = integer_sequence(shared->offset_variable_T_P_1 + 1, shared->dim_T_P_1);
  index[35] = integer_sequence(shared->offset_variable_T_N_1 + 1, shared->dim_T_N_1);
  index.names() = nms;
...
cpp11::sexp dust_seirhdagevaxmultistrainserotimedepbeta_capabilities() {
  return dust::r::dust_capabilities<seirhdagevaxmultistrainserotimedepbeta>();
}

cpp11::sexp dust_seirhdagevaxmultistrainserotimedepbeta_gpu_info() {
  return dust::gpu::r::gpu_info();
}
using model_cpu = dust::dust_cpu<seirhdagevaxmultistrainserotimedepbeta>;

SEXP dust_cpu_seirhdagevaxmultistrainserotimedepbeta_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
                             cpp11::sexp r_n_particles, size_t n_threads,
                             cpp11::sexp r_seed, bool deterministic,
                             cpp11::sexp gpu_config) {
  return dust::r::dust_cpu_alloc<seirhdagevaxmultistrainserotimedepbeta>(r_pars, pars_multi, step, r_n_particles,
