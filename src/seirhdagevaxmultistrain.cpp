...
[[cpp11::register]]
SEXP dust_cpu_seirhdagevaxmultistrain_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
...
[[cpp11::register]]
SEXP dust_cpu_seirhdagevaxmultistrain_update_state(SEXP ptr, SEXP r_pars, SEXP r_state,
...
[[cpp11::register]]
SEXP dust_cpu_seirhdagevaxmultistrain_filter(SEXP ptr, SEXP step_end,
...
// [[dust::class(seirhdagevaxmultistrain)]]
...
public:
  ...
  seirhdagevaxmultistrain(const dust::pars_type<seirhdagevaxmultistrain>& pars) :
    shared(pars.shared), internal(pars.internal) {
  }
  size_t size() {
    return shared->dim_D + shared->dim_E + shared->dim_G + shared->dim_H + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_vaccine_missed_doses + 13;
  }
  std::vector<real_type> initial(size_t step) {
    std::vector<real_type> state(shared->dim_D + shared->dim_E + shared->dim_G + shared->dim_H + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_vaccine_missed_doses + 13);
...
namespace dust {
template<>
dust::pars_type<seirhdagevaxmultistrain> dust_pars<seirhdagevaxmultistrain>(cpp11::list user) {
  using real_type = typename seirhdagevaxmultistrain::real_type;
  auto shared = std::make_shared<seirhdagevaxmultistrain::shared_type>();
  seirhdagevaxmultistrain::internal_type internal;
  ...
  shared->offset_variable_D = shared->dim_E + shared->dim_G + shared->dim_H + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_vaccine_missed_doses + 13;
  shared->offset_variable_E = shared->dim_N_tot + shared->dim_S + shared->dim_vaccine_missed_doses + 13;
  shared->offset_variable_G = shared->dim_E + shared->dim_H + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_vaccine_missed_doses + 13;
  shared->offset_variable_H = shared->dim_E + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_vaccine_missed_doses + 13;
  shared->offset_variable_I_A = shared->dim_E + shared->dim_I_P + shared->dim_N_tot + shared->dim_S + shared->dim_vaccine_missed_doses + 13;
  shared->offset_variable_I_C = shared->dim_E + shared->dim_I_A + shared->dim_I_P + shared->dim_N_tot + shared->dim_S + shared->dim_vaccine_missed_doses + 13;
  shared->offset_variable_I_P = shared->dim_E + shared->dim_N_tot + shared->dim_S + shared->dim_vaccine_missed_doses + 13;
  shared->offset_variable_R = shared->dim_E + shared->dim_I_A + shared->dim_I_C + shared->dim_I_P + shared->dim_N_tot + shared->dim_S + shared->dim_vaccine_missed_doses + 13;
  shared->offset_variable_vaccine_missed_doses = shared->dim_N_tot + shared->dim_S + 13;
  ...
  return dust::pars_type<seirhdagevaxmultistrain>(shared, internal);
}
template <>
cpp11::sexp dust_info<seirhdagevaxmultistrain>(const dust::pars_type<seirhdagevaxmultistrain>& pars) {
  const seirhdagevaxmultistrain::internal_type internal = pars.internal;
  const std::shared_ptr<const seirhdagevaxmultistrain::shared_type> shared = pars.shared;
  cpp11::writable::strings nms({"time", "H_inc", "D_inc", "H_inc_0_39", "H_inc_40_49", "H_inc_50_59", "H_inc_60_69", "H_inc_70_plus", "D_inc_0_39", "D_inc_40_49", "D_inc_50_59", "D_inc_60_69", "D_inc_70_plus", "N_tot", "S", "vaccine_missed_doses", "E", "I_P", "I_A", "I_C", "R", "H", "G", "D"});
  ...
  dim[15] = cpp11::writable::integers({shared->dim_vaccine_missed_doses_1, shared->dim_vaccine_missed_doses_2});
  dim[16] = cpp11::writable::integers({shared->dim_E_1, shared->dim_E_2, shared->dim_E_3});
  dim[17] = cpp11::writable::integers({shared->dim_I_P_1, shared->dim_I_P_2, shared->dim_I_P_3});
  dim[18] = cpp11::writable::integers({shared->dim_I_A_1, shared->dim_I_A_2, shared->dim_I_A_3});
  dim[19] = cpp11::writable::integers({shared->dim_I_C_1, shared->dim_I_C_2, shared->dim_I_C_3});
  dim[20] = cpp11::writable::integers({shared->dim_R_1, shared->dim_R_2, shared->dim_R_3});
  dim[21] = cpp11::writable::integers({shared->dim_H_1, shared->dim_H_2, shared->dim_H_3});
  dim[22] = cpp11::writable::integers({shared->dim_G_1, shared->dim_G_2, shared->dim_G_3});
  ...
  index[15] = integer_sequence(shared->offset_variable_vaccine_missed_doses + 1, shared->dim_vaccine_missed_doses);
  index[16] = integer_sequence(shared->offset_variable_E + 1, shared->dim_E);
  index[17] = integer_sequence(shared->offset_variable_I_P + 1, shared->dim_I_P);
  index[18] = integer_sequence(shared->offset_variable_I_A + 1, shared->dim_I_A);
  index[19] = integer_sequence(shared->offset_variable_I_C + 1, shared->dim_I_C);
  index[20] = integer_sequence(shared->offset_variable_R + 1, shared->dim_R);
  index[21] = integer_sequence(shared->offset_variable_H + 1, shared->dim_H);
  index[22] = integer_sequence(shared->offset_variable_G + 1, shared->dim_G);
...
cpp11::sexp dust_seirhdagevaxmultistrain_capabilities() {
  return dust::r::dust_capabilities<seirhdagevaxmultistrain>();
}

cpp11::sexp dust_seirhdagevaxmultistrain_gpu_info() {
  return dust::gpu::r::gpu_info();
}
using model_cpu = dust::dust_cpu<seirhdagevaxmultistrain>;

SEXP dust_cpu_seirhdagevaxmultistrain_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
                             cpp11::sexp r_n_particles, size_t n_threads,
                             cpp11::sexp r_seed, bool deterministic,
                             cpp11::sexp gpu_config) {
  return dust::r::dust_cpu_alloc<seirhdagevaxmultistrain>(r_pars, pars_multi, step, r_n_particles,
