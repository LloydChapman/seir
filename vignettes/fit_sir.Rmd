...
# data_raw <- data.frame(time = seq(1,n_steps*dt),cases = incidence)
# data <- dust::dust_data(data_raw)

plot(data_raw$day,data_raw$cases,type = "l",xlab = "Day",ylab = "Cases")
# plot(data_raw$time,data_raw$cases,type = "l",xlab = "Day",ylab = "Cases")
```
...
```{r}
# Run tuned MCMC
control_tuned_run <- mcstate::pmcmc_control(
    1000,
    save_state = TRUE,
    save_trajectories = TRUE,
    progress = TRUE)
pmcmc_tuned_run <- mcstate::pmcmc(mcmc_pars, filter, control = control_tuned_run)
```
