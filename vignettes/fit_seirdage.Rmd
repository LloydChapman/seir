...
First, we important demographic and social mixing data from the [`socialmixr` package](https://epiforecasts.io/socialmixr/). This contains a range of datasets, but here we use the 2008 POLYMOD survey from the UK, with ages defined in 10 year bands.
```{r}
...
transmission <- contact$matrix/rep(round(contact$demography$population/1e4), each = ncol(contact$matrix))
...
With the model specified, we then simulate an epidemic forward over 1000 days and output age-stratified trajectories for the susceptible, exposed, infected, recovered, and cumulative deaths compartments.
```{r}
...
dt <- 0.25
S_ini <- round(contact$demography$population/1e4)
...
# incidence <- rpois(length(incidence),incidence)
...
```
We then define a function to allow us to compare a model output (`state`) with observed data (`observed`) using a Poisson observation model:
```{r}
...
Next, we define a particle filter that propagates a set of particles of size `n_particles` over time, with each particle representing a single output from the model (`seirdage`). At each time step, the particle filter uses the comparison function (`compare`) and observed data (`seirdage`) to weight and resample the particles.
```{r}
...
# sigma <- mcstate::pmcmc_parameter("sigma",0.3,min = 0,
...
# make_transform <- function(pars,dt,S_ini,E_ini,I_ini,gamma,m,p_death,N_age){
# make_transform <- function(pars,dt,S_ini,E_ini,I_ini,sigma,gamma,m,p_death,N_age){
    function(pars){
        beta <- pars[["beta"]]
        gamma <- pars[["gamma"]]
        # sigma <- pars[["sigma"]]
...
# transform <- make_transform(pars,dt,S_ini,E_ini,I_ini,gamma = 0.1,transmission,p_death,N_age)
# transform <- make_transform(pars,dt,S_ini,E_ini,I_ini,sigma = 0.5,gamma = 0.1,transmission,p_death,N_age)

proposal_matrix <- matrix(c(0.003^2,0,0,0.01^2),nrow = 2,ncol = 2,byrow = TRUE)
# proposal_matrix <- matrix(c(0.01^2,0,0,0.05^2),nrow = 2,ncol = 2,byrow = TRUE)
# proposal_matrix <- matrix(0.002^2,nrow = 1,ncol = 1)
...
gamma <- mcstate::pmcmc_parameter("gamma",pmcmc_run$pars[nrow(pmcmc_run$pars),"gamma"],
                        min = 0,prior = function(x) dgamma(x,shape = 1,scale = 1,log = TRUE))
# sigma <- mcstate::pmcmc_parameter("sigma",pmcmc_run$pars[nrow(pmcmc_run$pars),"sigma"],
#                         min = 0,prior = function(x) dgamma(x,shape = 1,scale = 1,log = TRUE))
...
```{r}
# Plot sampled incidence curves on top of data
plot_incidence(pmcmc_run1$trajectories$state[,101:1000,1:100],incidence[2:100],data_raw$day[2:100])
```
