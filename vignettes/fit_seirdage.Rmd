...
Next, we define the model and its inputs. The model `seirdage` is a pre-compiled [`odin` model](https://mrc-ide.github.io/odin/) generated from the odin code defined in [inst/odin/seirdage.R](inst/odin/seirdage.R). We use data on the infection fatality risk (IFR) from [O'Driscoll et al (2020)](https://www.nature.com/articles/s41586-020-2918-0). We also define initial conditions for the model; if users require parameters for a known pathogen, these could potentially be sourced from the [`epiparameter` package](https://epiverse-trace.github.io/epiparameter/) using the `epidist_db` and `get_parameters` functions.
